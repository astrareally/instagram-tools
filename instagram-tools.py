...
def check(email):
  ...
  try:
   ...
   #   print(response.text)
      if 'email_is_taken' in str(response.text):Astra(email)
   ...
   #   print(str(response))
      if '"user":true' in response:Astra(email)
      else:bads_instgram+=1
  except:''
   # check(email)
  os.system('clear' if os.name == 'posix' else 'cls')
...
def qqq():
  while True:
    try:
      ...
      username = requests.post('https://www.instagram.com/api/graphql', headers=headers, data=data).json()['data']['user']['username']#['@Astra']
